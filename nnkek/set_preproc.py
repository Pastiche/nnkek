...
def train_test_val_split(df, frac, random_state=42):
    ...
    _, test_set = train_test_split(df, test_size=frac, random_state=random_state)

    train_set, val_set = train_test_split(_, test_size=test_set.shape[0], random_state=random_state)
    val_set["scope"] = "val"
